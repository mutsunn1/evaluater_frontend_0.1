<template>
  <aside :class="['flex flex-col bg-gray-900 text-white transition-all duration-300', expanded ? 'w-64' : 'w-12']">
    <!-- Toggle button -->
    <button
      class="flex h-12 items-center justify-center border-b border-gray-700 hover:bg-gray-800"
      @click="expanded = !expanded"
    >
      <svg v-if="!expanded" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="expanded" class="flex-1 overflow-y-auto p-4 space-y-6">
      ...
      <div class="text-center">
        ...
        <div class="text-xs text-gray-400">{{ profile.hsk_level > 1 ? '当前等级' : '等待评测' }}</div>
      </div>

      <!-- Skill Bars -->
      <div class="space-y-3">
        <h4 class="text-xs font-semibold uppercase tracking-wider text-gray-500">能力维度</h4>
        <div v-for="skill in skills" :key="skill.key" class="space-y-1">
          <div class="flex items-center justify-between text-xs">
            <span>{{ skill.label }}</span>
            <span class="font-mono">{{ hasData ? (profile.skill_levels?.[skill.key] || 0) + '%' : '未知' }}</span>
          </div>
          <div class="h-2 overflow-hidden rounded-full bg-gray-700">
            ...
            />
            <div v-else class="h-full rounded-full bg-gray-600 animate-pulse" style="width: 0%" />
          </div>
        </div>
      </div>
      ...
      <div v-if="profile.strengths?.length" class="space-y-2">
        <h4 class="text-xs font-semibold uppercase tracking-wider text-gray-500">已掌握</h4>
        <div class="flex flex-wrap gap-1.5">
          <span v-for="s in profile.strengths" :key="s" class="rounded-full bg-green-900/50 px-2 py-0.5 text-xs text-green-300">
            {{ s }}
          </span>
        </div>
      </div>
      ...
      <div v-if="profile.next_focus?.length" class="space-y-2">
        <h4 class="text-xs font-semibold uppercase tracking-wider text-gray-500">建议关注</h4>
        <div class="flex flex-wrap gap-1.5">
          <span v-for="f in profile.next_focus" :key="f" class="rounded-full bg-yellow-900/50 px-2 py-0.5 text-xs text-yellow-300">
            {{ f }}
          </span>
        </div>
      </div>
