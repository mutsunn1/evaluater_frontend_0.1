...
const steps: ThinkingStep[] = [
  { agent: '题目摘要', agent_key: 'thinking_coordinator', output: '系统正在选择适合当前水平的语法题。' },
  { agent: '出题智能体', agent_key: 'grammar_generator', output: '本题围绕把字句的语序、结构和语义是否成立进行辨析。' },
];
...
describe('ThinkingSidebar mobile full-screen', () => {
  ...
  it('renders all thinking step cards with agent names and output', () => {
    ...
    expect(wrapper.text()).toContain('本题围绕把字句的语序、结构和语义是否成立进行辨析。');
  });
  ...
  it('title is visible as "思考过程"', () => {
    ...
    expect(wrapper.find('h3').text()).toBe('思考过程');
  });
});
