...
const allAnswered = computed(() => {
  ...
  return props.data.sub_questions.every((sq) =>
    subAnswers.value[sq.sub_id]?.trim()
  );
});

function trySubmitAll() {
  if (!allAnswered.value) return;
  const combined = props.data
    .sub_questions!.map((sq) => `[${sq.sub_id}] ${subAnswers.value[sq.sub_id]}`)
    .join("\n");
  ...
  <div>
    ...
    <div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
      <div class="mb-2 text-xs font-semibold text-gray-500">
        {{ t("chat.question.readingPassage") }}
      </div>
      <p
        class="whitespace-pre-wrap break-words text-base leading-relaxed text-gray-800"
      >
    ...
    <!-- Sub-questions -->
    <div
      v-if="data.sub_questions && data.sub_questions.length"
      class="space-y-4"
    >
      <div
        v-for="sq in data.sub_questions"
        :key="sq.sub_id"
        class="rounded-lg border border-gray-200 p-3"
      >
        <div class="mb-2 text-xs font-semibold text-gray-500">
          {{ t("chat.question.subQuestion", { id: sq.sub_id }) }}
        </div>
